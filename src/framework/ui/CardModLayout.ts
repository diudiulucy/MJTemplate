...
module LC {
    ...
    */
    export class CardModLayout extends eui.Component {
        ...
        public direction: LC.Directions;
         ...
         * @param bu_cards    补牌
         */
        public appliqueCards(direction: LC.Directions, hua_card: Array<number>, bu_cards: Array<number>) {
            if (direction != LC.Directions.Down) return;//其他的人的补花不做任何处理
            //删除花牌
         ...
         * 打出一张牌
         */
        public outACard(direction: LC.Directions, cardValue: number, cardPointer: LC.CardPointer) {
         ...
         * 根据值来删除所有的手牌 包括摸的牌
         */
        private _deleteAllHandCardsByValue(direction: LC.Directions, cardValue: number) {
        ...
        * 根据牌值删除手牌
        */
        private _deleteHandCardByValue(direction: LC.Directions, cardValue: number) {
            if (direction != LC.Directions.Down) {//其他方向服务器不给值，没办法删，就用此方法删除
        ...
        */
         ...
         * 移动指针
         */
        private _moveCardPointer(cardContainer: eui.Group, cardPointer: LC.CardPointer) {
        ...
        * @param card        牌对象
        */
        private _adjustOutDir(direction: LC.Directions, card: LC.Card) {
            switch (direction) {
        ...
        */
        private _handCardHandler(event: egret.Event) {
            ...
            if (this._upCardContainer != cardContainer) {//有弹起的牌
                ...
                let card = <LC.Card>this._upCardContainer.getChildAt(0);
                this.canOutACard = false;
                EventManager.getInstance().dispatchCustomEvent(LC.SelectCardComplete, { cardValue: card.value });
            }
        }
